...
{
    public class Top100DBMongo : IDisposable
    {
        private readonly IStore store;
        private readonly IList<Top100Common.Song> songs;

        public Top100DBMongo()
        {
            var mongoConnectionString = config["MONGO_CONNECTION_STRING"];

            store = new Store(mongoConnectionString);
            songs = store.SearchAllAsync(CancellationToken.None).Result;
        }
